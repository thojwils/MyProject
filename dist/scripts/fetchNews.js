...
const fetchNews = async () => {
  try {
    const res = await fetch("https://bing-news-search1.p.rapidapi.com/news/search?" + `q=${searchTerm}` + `&freshness=Day&textFormat=Raw&safeSearch=Strict`, options); // News API Version
    // const fetchNews = async () => {
    //   try {
    ...
    const articles = data.value; //Time for calc hours since post

    for (let i = 0; i < 20; i++) {
      console.log(articles[i]); // console.log([articles[i].title, articles[i].url]);
      ...
      const time = hours > 0 ? hours % 24 + " hour's ago" : minutes % 60 + " minute's ago";
  ...
  <div class="card">
    <div class="card__header">
     ...
     <img src="${articles[i].image.thumbnail.contentUrl}" alt="" onmousehover="imageQualityWarning" class="card__image" width="600">
     </div>  
    </div>
    <div class="card__body">
    <div>
    <span id="left" class="tag tag-blue">${articles[i].category ? articles[i].category : "Uncategorized"}</span>
    <span id="right" class="tag tag-red">${searchTerm}</span>
    </div>  
      <h6>${articles[i].name.length > 80 ? articles[i].name.substring(0, 80) + "..." : articles[i].name}</h6>
    </div>
...
<div>
    ...
    console.error(err); // .then((data) => data.json())
    // .then((data) => {
